import React, { useState, useMemo } from "react";
import {
  View,
  StyleSheet,
  Text,
  StatusBar,
  SafeAreaView,
  Image,
  ScrollView,
} from "react-native";
import TopHeader from "../../component/TopHeader";
import colors from "../../config/colors";
import { connect } from "react-redux";
import { CATEGORY } from "../../config/enums";
import { moderateScale } from "react-native-size-matters";
import CompleteDetail from "../../component/Spa/CompleteDetail";
import SpaFooter from "../../component/Spa/spaFooter";

function ForYouCompleteDetail(props) {
  const [forYouDetail, setForYouDetail] = useState(
    props.route.params.forYouDetail
  );
  const [isPackageSelected, setIsPackageSelected] = useState(false);

  useMemo(() => {
    forYouDetail.quantity = 0;
  }, []);

  const handleNextButton = () => {
    navigation.navigate("ForYouOrder", {
      data: forYouDetail,
      treatmentName: props.route.params.treatmentName,
      itemName: forYouDetail.item_name,
      itemPrice: forYouDetail.calculatePrice
        ? parseFloat(forYouDetail.calculatePrice).toFixed(2)
        : parseFloat(forYouDetail.price).toFixed(2),
    });
    const forYouOrder = [];
    let cartItem = {
      id: props.route.params.forYouDetail.id,
      qty: forYouDetail.quantity,
      settings: {
        type_of_treatment: props.route.params.packageName,
      },
    };
    forYouOrder.push(cartItem);
    props.cartChange(forYouOrder);
  };

  const incrementCartProduct = () => {
    var forYOuServiceCart = forYouDetail;
    forYOuServiceCart.quantity += 1;
    let price =
      Number(forYOuServiceCart.price) * Number(forYOuServiceCart.quantity);
    forYOuServiceCart.calculatePrice = price;
    setForYouDetail((forYouDetail) => ({
      ...forYouDetail,
      ...forYOuServiceCart,
    }));
    if (forYouDetail.quantity >= 0) {
      setIsPackageSelected(true);
    }
  };
  //remove Item to Cart
  const decrementCartProduct = () => {
    var forYOuServiceCart = forYouDetail;
    if (forYOuServiceCart.quantity > 0) forYOuServiceCart.quantity -= 1;
    let price =
      Number(forYOuServiceCart.price) * Number(forYOuServiceCart.quantity);
    forYOuServiceCart.calculatePrice = price;
    setForYouDetail((forYouDetail) => ({
      ...forYouDetail,
      ...forYOuServiceCart,
    }));
    if (forYouDetail.quantity <= 0) {
      setIsPackageSelected(false);
    }
  };
  return (
    <SafeAreaView style={styles.container}>
      <StatusBar
        backgroundColor={colors.primary}
        barStyle="dark-content"
        translucent={true}
        hidden={false}
      />
      <TopHeader
        title={"FOR YOU"}
        headerImage={require("../../assets/TopHeader/sideIcon.png")}
        logoImage={require("../../assets/TopHeader/forYou.png")}
        type={CATEGORY.ROOM_SERVICE}
        {...navigation}
        rightImage={require("../../assets/TopHeader/back-arrow.png")}
        leftImagePress={() => navigation.goBack()}
      />
      <ScrollView style={{ flexGrow: 1 }}>
        <View style={styles.selectView}>
          <Text style={styles.allPackage}>Selected Treatments</Text>
        </View>
        <CompleteDetail
          image={{
            uri:
              forYouDetail.imgurl == " "
                ? "data:image/jpeg;base64,"
                : forYouDetail.imgurl,
          }}
          name={forYouDetail.item_name}
          showQuantity={forYouDetail.quantity == 0 ? " " : "X"}
          description={forYouDetail.description}
          price={
            forYouDetail.calculatePrice
              ? parseFloat(forYouDetail.calculatePrice).toFixed(2)
              : parseFloat(forYouDetail.price).toFixed(2)
          }
          quantity={forYouDetail.quantity == "0" ? " " : forYouDetail.quantity}
          incrementCart={() => incrementCartProduct()}
          cartDecrement={() => decrementCartProduct()}
        />
      </ScrollView>
      <SpaFooter
        nextButtonPress={() => handleNextButton()}
        addTreatmentPress={() => navigation.navigate("ForYouScreen")}
        disabled={!isPackageSelected}
        disabledStyle={isPackageSelected}
      />
    </SafeAreaView>
  );
}

const styles = StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: colors.Gray,
    paddingTop: Platform.OS === "android" ? StatusBar.currentHeight : 0,
  },
  selectView: {
    flexDirection: "row",
    justifyContent: "space-between",
    width: "92%",
    alignSelf: "center",
    marginTop: moderateScale(20),
  },
  allPackage: {
    color: colors.darkGray,
    fontSize: moderateScale(14),
    left: moderateScale(14),
  },
  nextButton: {
    alignItems: "center",
    width: moderateScale(150),
    marginLeft: moderateScale(20),
    padding: moderateScale(14),
    backgroundColor: colors.primary,
    borderRadius: 7,
  },
});
const mapDispatchToProps = (dispatch) => {
  return {
    cartChange(cart) {
      dispatch({
        type: "ADD_TO_FOR_YOU_CART",
        payload: cart,
      });
    },

    initforYou(data) {
      dispatch({
        type: "INIT_FOR_DATA",
        payload: data,
      });
    },
  };
};

const mapStateToProps = (state) => {
  return {
    forYou: state.forYou,
    user: state.user,
    roomServiceCart: state.roomServiceCart,
  };
};

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(ForYouCompleteDetail);
